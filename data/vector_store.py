...
class AgentMemoryStore:
    ...
    def _build_context_text(self, state: dict[str, Any]) -> str:
        ...
        return (
            f"Price: {price}, VIX: {vix}, MACD Hist: {macd_hist}. News: {news_titles}"
        )
    ...
    def query_similar_conditions(
        self, state: dict[str, Any], n_results: int = 3
    ) -> list[dict]:
        ...
        try:
            results = self.collection.query(
                query_texts=[query_text], n_results=n_results, where={"symbol": symbol}
            )
